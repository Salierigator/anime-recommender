...
df = pd.read_csv(SRC, usecols=["popularity"])
n = len(df)
...
print_engagement("popularity", df["popularity"])
print()

print("=" * 50)
print("POPULARITY semantic check")
print("=" * 50)
pop = df["popularity"].dropna()
print(f"unique values: {pop.nunique():,}  (nếu == total -> rank-based)")
print(f"min: {pop.min():.0f}, max: {pop.max():.0f}")
print(f"is integer: {bool((pop == pop.astype(int)).all())}")
print(f"first 5: {pop.head(5).tolist()}")

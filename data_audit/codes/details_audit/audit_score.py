...
df = pd.read_csv(SRC, usecols=["score"])
n = len(df)
...
print_numeric("score", df["score"])

score = df["score"].dropna()
n_outrange = int(((score < 1) | (score > 10)).sum())
print(f"\nValues outside [1, 10]: {n_outrange:,}")
if n_outrange > 0:
    bad = score[(score < 1) | (score > 10)]
    print(f"  sample: {bad.head(10).tolist()}")

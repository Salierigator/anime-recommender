...
df = pd.read_csv(SRC, usecols=["rank"])
n = len(df)
...
print_numeric("rank", df["rank"])

rk = df["rank"].dropna()
print(f"\nrank unique:        {rk.nunique():,}")
print(f"rank n_total:       {len(rk):,}  (nếu unique = total -> rank consecutive)")
print(f"rank is integer-like: {bool((rk == rk.astype(int)).all())}")

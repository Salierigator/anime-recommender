...
trong checkpoint + hằng số path chung trong model/retriever/src/config.py.

    python model/retriever/export.py [--ckpt model/retriever/checkpoints/best.pt]
                               [--out artifacts] [--device cpu]

Ghi (contract thesis-final:docs/PROJECT_STRUCTURE.md §4):
...
Tự sinh bởi `model/retriever/export.py` — **không sửa tay**. Rerun export để cập nhật.
...
def main() -> None:
    root = config.ROOT.parent.parent      # model/retriever/ → repo root

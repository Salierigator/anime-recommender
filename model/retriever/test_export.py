...
Đọc kết quả:
  ...
  - test_cold KHÔNG đụng ở đây (final exam, chấm 1 lần khi chốt model).
    ...
    python model/retriever/test_export.py [--out artifacts] [--device cpu]
"""
...
class ArtifactEncoder:
    ...
    model/ranker/src/user_encode.py::UserEncoder (cả hai phản chiếu cùng logic train)."""
...
def validate_ranker_exports(out: Path) -> None:
    ...
    assert not (out / "eval_queries_test_cold.parquet").exists(), (
        ...
        "pipeline; rerun `model/retriever/export.py` (không --final-exam) để xoá.")
...
def main():
    ...
    ap.add_argument("--out", type=Path, default=config.ROOT.parent.parent / "artifacts")

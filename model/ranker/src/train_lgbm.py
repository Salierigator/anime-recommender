...
(model/ranker/train-data). Train = datasets/train.parquet (label graded 0-4, group = user, top-200 pool);
...
Fix so với ranker cũ: num_boost_round 4000 + early_stopping(100) (cũ cap 1000 chưa hội tụ).
    ...
    venv/bin/python model/ranker/src/train_lgbm.py --smoke     # slice nhỏ local, kiểm tra end-to-end
"""

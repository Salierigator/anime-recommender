...
sys.path (data_prep/ + entrypoint top-level chèn src/ vào path). Firewall: ranker chỉ ĐỌC
`artifacts/` + `data/cleaned/{details,profiles}.csv` (file nhỏ) — KHÔNG ratings.csv,
KHÔNG model/retriever/train-data (mọi thứ cần đã được model/retriever/export.py đẩy sang artifacts).
"""
from __future__ import annotations

from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent          # model/ranker/
REPO = ROOT.parent.parent                              # anime recommender/ (repo root)

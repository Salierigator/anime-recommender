...
Modes:
  ...
  (default)         Load model trong model/ranker/models/ (*.txt LightGBM) →
                    sweep blend α trên VAL → Pareto-select vs cosine → report TEST + val_cold
                    → ghi model/ranker/models/eval_selection.json (export.py đọc).
...
def predict(model_path: Path, df: pl.DataFrame) -> np.ndarray:
    """Score pool bằng LightGBM booster (.txt)."""
...
def main() -> None:
    ...
    ap.add_argument("--models", nargs="*", type=Path,
                    help="default: mọi *.txt trong model/ranker/models/")
...
def run_final_exam() -> None:
    ...
    assert (config.ARTIFACTS / "eval_queries_test_cold.parquet").exists(), \
        "cần `venv/bin/python model/retriever/export.py --final-exam` trước"
    ...
    base = eval_pool(cos, labels, offsets, r_total, config.KS, pooled=True,
                     label_liked=ll, r_liked=rl)
    pred = predict(Path(sel["model"]), df)

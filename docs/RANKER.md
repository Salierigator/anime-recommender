...
**Held-out xác nhận — test_cold final exam (chấm ĐÚNG 1 lần, 2026-06-18, serve-path):** kênh
phục vụ cold = cosine retriever, đo 2 chế độ trên test_cold (8.510 user; `docs/RESULTS.md §7`):
- **full-catalog** (rank giữa toàn 22.8k): r@100 .3414 / r@200 .4710 / ndcg@10 .1397 — khớp
  val_cold (.3373/.4664/.1398) + khớp đúng cosine baseline ranker → generalize, không overfit.
- **honly** (rank chỉ giữa 1.142 item mới H = đúng UX "Anime mới"): r@100 **.6755** / r@200
  **.8261** / ndcg@10 **.2368** / liked_r@100 **.7306** — khớp val_cold honly (.6726/.8234). Đây
  là số trung thực của section cold (chỉ hiển thị anime mới, không đấu với warm).
- ép cold qua blend α=1 (diagnostic): ndcg@10 → **.0000** — lặp lại val_cold (.1398→.0002),
  khẳng định quyết định ③ tách kênh.

Số đầy đủ: `eval_selection.json::{baseline_test_cold, test_cold_metrics}` (ranker) +
`artifacts/test_cold_final_exam.json` (retriever full+honly+liked).

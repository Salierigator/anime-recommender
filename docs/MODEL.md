...
Doc tổng hợp cho `model/` — giải thích **build cái gì, set up thế nào, vì sao**. Dựng kiến trúc Two-Tower (PyTorch) + training pipeline ăn artifacts `train-data/` (xem `docs/TRAIN_DATA.md`).
...
Convention path: `.py` dùng `ROOT = Path(__file__).resolve().parent.parent`; artifacts ở `ROOT/"train-data"`. Notebook (trong `model/`) tự thêm `sys.path` rồi `import config, data, model as M, loss, metrics, train`.
...
- **logQ** (`train-data/logq.npy`, popularity item-as-positive) trừ vào in-batch (gồm positive). Thiếu → retriever đè item phổ biến bệnh lý.

...
- **Retrieval** (`retriever/`) — Two-Tower (PyTorch) học user/item embedding, ANN top-K cosine từ ~22.8k anime. *Đã có checkpoint, đang tune trên Colab.*
- **Ranking** (`ranker/`) — GBDT rerank top-K của retriever bằng feature chi tiết. *Làm sau.*
...
```
anime-recommender/
├── CLAUDE.md                 # guideline + context tổng (mỗi mảng có CLAUDE.md riêng — làm sau)
├── PROJECT_STRUCTURE.md      # file này
├── requirements.txt          # deps chung (torch + lightgbm + fastapi…)
...
│   ├── codes/                #   {details,profiles,ratings}_audit/audit_*.py — check lẻ từng cột
│   ├── output/               #   *_audit/audit_*.txt — kết quả audit
│   └── refresh_samples.py    #   sync data-sample/ ← cleaned-data/
├── docs/                     # CLEANING.md · TRAIN_DATA.md · TWO_TOWER_MODEL.md
│
├── artifacts/                # ★ FIREWALL: retriever GHI, ranker+service ĐỌC (xem §4)
│
├── retriever/                # [ĐÃ CÓ] Two-Tower retrieval
│   ├── src/                  #   config/data/model/loss/metrics/train.py (+__init__) — import flat, CWD=src
│   ├── baselines/            #   _eval + popular/rand/mf/itemknn/content_based
│   ├── data_prep/            #   01..06 + 99_verify (cleaned-data → train-data)
│   ├── train-data/           #   artifacts train-data (gitignored)
│   ├── checkpoints/          #   best.pt (gitignored; persist Drive/Colab)
│   └── train.ipynb           #   notebook điều khiển train (Colab)
│
├── ranker/                   # [LÀM SAU] GBDT reranker
│   └── src/
│
...
- **Retriever** *(đã có)*: tiếp tục tune trên Colab; xong chạy lại `export.py` ghi đè `artifacts/`.
- **Bước unblock** (làm trước): viết `docs/CONTRACT.md` + `retriever/export.py`, export từ `best.pt` → `artifacts/` (provisional) → unblock cả ranker lẫn service.
- **Ranker** *(làm sau)*: feature pipeline (raw `cleaned-data/` + cosine từ `artifacts/`) → train GBDT → `artifacts/ranker.txt`. Retriever chốt → trỏ artifacts mới + retrain (CPU, vài phút).
- **Service** *(làm sau)*: backend đọc `artifacts/` + Jikan/MAL → 2 user-flow; frontend. Ban đầu dùng ranker stub, cuối swap ranker thật.

**Việc còn hoãn:** `retriever/export.py` (code thật), `ranker/` (GBDT), `service/` (backend + frontend), two-stage eval harness (ghép retriever+ranker — đặt trong `ranker/` hoặc `eval/`, quyết sau).

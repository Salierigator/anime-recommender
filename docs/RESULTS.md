...
>
> ✅ **Re-export DONE (2026-06-17)**: `best.pt`/`artifacts/`/`eval_reference.json` giờ là **`final`** (`CONTRACT.md` epoch=7 step=31500, synopsis OFF). **Serve-path official của `final` đã đo** → §3b (bảng serve-path). ⚠️ **Ranker vẫn STALE**: `ranker.txt` còn `v5_hist64_ep2` (train trên vectors cũ) → MỌI số two-stage §5–§8 (và §3 retriever cũ) **chưa khớp `final`** cho tới khi retrain ranker (`docs/RANKER.md §9`) rồi cập nhật file này.
...
- *Retriever-only (test, full catalog)* = `eval_reference.json::test_warm` (hiện = `final`: r@100 .5387, r@200 .6758, ndcg@10 .5323). Về nguyên tắc nó ≡ hàng `baseline_test` (cosine) trong `ranker_meta.json` **by construction** (pool two-stage = top-200 cosine, recall/ndcg@K trong pool ≡ full ranking khi K ≤ 200; sanity gate ép trùng trong 2e-3). ⚠️ **Tạm thời KHÔNG trùng**: `ranker_meta.json` còn là `v5_hist64_ep2` (cosine test .5160/.5155) — sẽ trùng lại sau khi retrain ranker trên pool `final`.
...
> ✅ Số `final` ở **§3b** giờ có **cả hai**: checkpoint-path (run-vs-run, `runs.csv`) **và serve-path official** (`eval_reference.json`, đo qua artifacts đã re-export). Lưu ý đặc thù `final`: serve-path ndcg@10 **cao hơn** checkpoint-path (.5323 vs .4242) — ngược chiều quy luật recall — vì `final` train `history_source=embed`: lúc eval-train (checkpoint-path) row H = id thật chưa train = noise-vector làm distractor đầu bảng → dìm ndcg@10; serve-path H→OOV (content) bớt distractor → ndcg@10 phục hồi. Recall vẫn theo quy luật cũ (serve hơi thấp hơn). Số serve-path two-stage §6/§7 vẫn là `v5_hist64_ep2` (STALE — ranker chưa retrain).
...
> 🔴 **STALE (pool `v5_hist64_ep2`)**. Re-tune trên pool `final` (2026-06-17, coarse 20 run): bảng + phân tích ở
> **`docs/RANKER_EXPERIMENTS.md`** — max ndcg@10 `lrank_t20_gainLin` .7134, max liked `xendcg_lr03` .5632, relabel
> liked-aware bị bác. CHƯA chốt (chờ confirm full). Số §5/§6 dưới = pool cũ, giữ tham chiếu.
